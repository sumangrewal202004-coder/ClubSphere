...
// Security: verify this manager actually owns this club
...
exports.updateApplicationStatus = async (req, res) => {
  ...
  try {
    ...
    // Get application + club info for notification
    const appInfo = await db.query(
      `SELECT a.student_id, c.name AS club_name, c.manager_id
       FROM applications a
    ...
    // Security: only the club's manager can update
    ...
    res.json({ message: `Application ${status}`, data: result.rows[0] });
  } catch (err) {
    console.error(err);
...
exports.getMyClubs = async (req, res) => {
  try {
    const result = await db.query(
      `SELECT 
        c.id, c.name, c.description, c.requirements,
