...
exports.createEvent = async (req, res) => {
  ...
  try {
    ...
    // ✅ MOVED: respond AFTER notifications are sent
    // ✅ Notify ALL students of the same college
try {
  // get the college this club belongs to
  const collegeRes = await db.query(
    `SELECT college_id FROM clubs WHERE id=$1`,
    [clubId]
  );
  const collegeId = collegeRes.rows[0].college_id;

  // get all students of that college
 const students = await db.query(
  `SELECT id FROM users 
   WHERE college_id=$1 AND id != $2`,
  [collegeId, req.user.id]
);
  console.log(`Notifying ${students.rows.length} students of college ${collegeId}`);
  ...
  for (const s of students.rows) {
    await db.query(
      ...
      [
        s.id,
        `New event at ${clubName}: "${title}" on ${new Date(event_date).toDateString()} at ${venue}`
      ]
    );
  }

  console.log('All student notifications sent');
...
exports.registerForEvent = async (req, res) => {
  ...
  try {
    ...
    const event = await db.query(
      `SELECT e.id, e.title, e.event_date, c.manager_id 
       FROM events e
       JOIN clubs c ON e.club_id = c.id
       WHERE e.id=$1`,
      [eventId]
    ...
    await db.query(
      `INSERT INTO notifications (user_id, message) VALUES ($1, $2)`,
      [event.rows[0].manager_id, `${studentName} has registered for your event "${event.rows[0].title}"`]
    );
